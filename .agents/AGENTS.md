...
- **Target Platform (Telegram Mini App):** The frontend is designed as a Telegram Mini App (TMA). 
  ...
  - **Authentication:** The frontend sends `initData` via POST to authenticate. The backend validates it and returns a Laravel Sanctum token. All subsequent API requests from Flutter must include this token in the `Authorization: Bearer <token>` header.
- **Key Product Pillars:**
...
- The workspace `/home/artem/mf_new` is the Laravel backend repository (Database: **MySQL**. Assume MySQL for any raw queries or indexing).
...
- When running Laravel Artisan commands, database migrations, seeds, or tests, execute them inside the Docker container using:
  ...
  - Run tests: `docker compose exec php php artisan test` (Standard PHPUnit is used, not Pest).
...
- **State Management:** Use **Provider** as the standard state management solution.
- **Testing:** Standard **flutter_test** is used. Avoid introducing external mocking libraries unless explicitly requested.
